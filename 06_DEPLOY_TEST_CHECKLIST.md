...
| 테스트 | 입력값 | 기대 결과 | 통과 |
|---|---|---|---|
| 긍정 문장 | `오늘은 정말 행복하고 만족스러워요.` | 긍정 + 초록색 배지 `#3D8B5A` | [ ] |
| 부정 문장 | `너무 실망스럽고 화가 납니다.` | 부정 + 붉은색 배지 `#A14A3B` | [ ] |
| 중립 문장 | `오늘 회의는 오후 3시에 시작합니다.` | 중립 + 회색 배지 `#6E7781` | [ ] |
...
| 감성 색상 배지 | 긍정은 초록색, 부정은 붉은색, 중립은 회색으로 표시 | [ ] |
| 색상 접근성 | 색상만으로 결과를 구분하지 않고 텍스트 라벨도 함께 표시 | [ ] |
| 신뢰도 | 0~100 사이 숫자와 `%` 표시 | [ ] |
| 신뢰도 바 | 백분율에 맞게 너비 변경, 색상은 감성 배지 색상과 일치 | [ ] |
| 분석 이유 | 한국어 1~2문장 표시 | [ ] |
| 결과 카드 | 성공 시 표시 | [ ] |
...
- [ ] 긍정 / 부정 / 중립 중 하나로 표시된다.
- [ ] 결과가 지정 색상 배지와 함께 표시된다.
- [ ] 신뢰도 바 색상이 결과 배지 색상과 일치한다.
- [ ] 신뢰도 백분율이 표시된다.
- [ ] 분석 이유가 표시된다.
- [ ] 오류 메시지가 표시된다.
- [ ] Supabase에 로그가 저장된다.
